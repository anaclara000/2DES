...
select * from Clientes c join Telefones t on c.Cliente_id = t.cliente_id;

create view vw_clientes as
select c.Cliente_id, c.nome, c.logradouro, c.numero, c.complemento, c.bairro, c.referencia, t.telefone as Telefones from Clientes c
inner join Telefones t on c.Cliente_id = t.cliente_id;

select * from vw_clientes where nome ="Cesar Augusto Pascali Rago";

delete from telefones where cliente_id = (select cliente_id from clientes where nome = "Cesar Augusto Pascali Rago");
insert into telefones values(12,"1999295540");

insert into clientes value(null,'Joaquim Inácio Silva','Boa vista',2035,'BL 23 APTO 09','Recanto camanducaia',null);
...
--VIEW DE ITENS 
select * from Itens_pedido I join pizzas P on I.pizza_id = P.pizza_id;
create view vw_itens as 
select I.pedido_id, I.pizza_id, P.nome, I.quantidade, I.valor as Pizzas from itens_pedido I
inner join Pizzas P on I.pizza_id = P.pizza_id;

--VIEW DE PEDIDOS
select * from pedidos P join pizzas PZ on P.pedido_id = PZ.pizza_id;
create view vw_pedidos as 
select P.pedido_id, P.valor ,P.cliente_id, P.data, P.hora, PZ.pizza_id, PZ.nome, PZ.valor as pizzas from pedidos P
inner join pizzas PZ on P.pedido_id = PZ.pizza_id;
...
select * from itens_pedido order by pedido_id desc limit 4;
