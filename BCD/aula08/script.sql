...
update Pedidos set valor=(select sum(quantidade * valor) from Itens_Pedido where pedido_id = 28) where pedido_id = 28;
...
create procedure insere_pedidos()
	...
	start transaction;
		insert into Pedidos values (default, 10, curdate(), curtime(),null);
...
drop procedure if exists insere_itensPedido;
delimiter //
create procedure insere_itensPedido()
	begin
	declare erro_sql tinyint default false;
	declare continue handler for sqlexception set erro_sql = true;
	start transaction;
		insert into Itens_Pedido values (29, 41, 5, 29.09);
        insert into Itens_Pedido values (29, 6, 3, 32.13);
        insert into Itens_Pedido values (29, 26, 5, 33.73);
		if erro_sql = false then
			commit;
			select 'Transação efetivada com sucesso' as Resultado;
		else
...
delimiter ;
call insere_itensPedido();
select * from Itens_Pedido;
update Pedidos set valor=(select sum(quantidade * valor) from Itens_Pedido where pedido_id = 29) where pedido_id = 29;
select * from pedidos;
